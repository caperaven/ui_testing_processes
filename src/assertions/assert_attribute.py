    ...
    for attr in attributes:
        ...
        if value != exp_value:
            ...
            await set_error(driver, results, args["step"],
                            "error: attribute '{}' on '{}' should have been '{}' but was '{}'".format(attr, name,
                                                                                                      exp_value, value))
            break
...
async def assert_attr_eq(driver, args, results):
    ...
    if value == exp_value:
        ...
        name = get_name(args)
        await set_error(driver, results, args["step"],
                        "error: attribute '{}' on '{}' should have been '{}' but was '{}'".format(args["attr"], name,
                                                                                                  exp_value, value))
...
async def assert_attr_neq(driver, args, results):
    ...
    if value == exp_value:
        name = get_name(args)
        await set_error(driver, results, args["step"],
                        "error: attribute '{}' on '{}' should NOT have been '{}'".format(args["attr"], name, exp_value))
    else:
        results[args["step"]] = {
            "result": "success",
            "memory": get_memory(driver)
...
async def assert_has_attr(driver, args, results):
    ...
    if value is None:
        await set_error(driver, results, args["step"],
                        "error: has_attribute '{}' should exist but does not".format(args["attr"]))
    else:
        results[args["step"]] = {
            "result": "success",
            "memory": get_memory(driver)
...
async def assert_has_not_attr(driver, args, results):
    ...
    if len(value) > 0:
        await set_error(driver, results, args["step"],
                        "error: has_attribute '{}' should not exist but does on '{}'".format(args["attr"],
                                                                                             args["query"]))
    else:
        results[args["step"]] = {
            "result": "success",
...
async def assert_has_class(driver, args, results):
    ...
    if value is not None and value.__contains__(exp_value):
        ...
        await set_error(driver, results, args["step"],
                        "error: expected class '{}' to exist on '{}'".format(exp_value, query))
...
async def assert_has_not_class(driver, args, results):
    ...
    if value is not None and value.__contains__(exp_value):
        await set_error(driver, results, args["step"],
                        "error: expected class '{}' to NOT exist on '{}'".format(exp_value, query))
    else:
        results[args["step"]] = {
            "result": "success",
            "memory": get_memory(driver)
